...
void LogitechZ906Component::set_switch_state(SwitchType type, bool state) {
  switch (type) {
    ...
    case POWER:
      ...
      ESP_LOGD(TAG, "publishing power state %s", this->state_.power ? "true" : "false");
      this->power_->publish_state(this->state_.power);
...
void LogitechZ906Component::set_power(bool power){
  ...
  if (power){
    ...
    this->power_amp_on();
    this->z906_.set_state(0 , (uint8_t)this->state_.bass_volume,(uint8_t) this->state_.rear_volume,(uint8_t) this->state_.center_volume, (uint8_t)this->state_.input, (uint8_t*)this->state_.effect);
