...
CONF_POWER_BINARY_OUTPUT_ID = "power_pin_id"
...
CONFIG_SCHEMA = cv.All(
    cv.Schema({
        ...
        cv.Required(CONF_POWER_BINARY_OUTPUT_ID): cv.use_id(BinaryOutput),
        })
...
async def to_code(config):
    ...
    power_output = await cg.get_variable(config[CONF_POWER_BINARY_OUTPUT_ID])
    cg.add(var.set_amplifier_uart_parent(amplifier_uart))
    cg.add(var.set_binary_output_parent(power_output))
